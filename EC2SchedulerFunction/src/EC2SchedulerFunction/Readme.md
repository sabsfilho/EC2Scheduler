...
WORKDIR /src
COPY ["EC2SchedulerFunction.csproj", "EC2SchedulerFunction/"]
RUN dotnet restore "EC2SchedulerFunction/EC2SchedulerFunction.csproj"

WORKDIR "/src/EC2SchedulerFunction"
COPY . .
RUN dotnet build "EC2SchedulerFunction.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "EC2SchedulerFunction.csproj" \
...
Execute unit tests
```
cd "EC2SchedulerFunction/test/EC2SchedulerFunction.Tests"
dotnet test
```
